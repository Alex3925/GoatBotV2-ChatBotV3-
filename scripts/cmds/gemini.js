...
async function b(c, d, e, f) {
  try {
    const g = await axios.get(`https://deku-rest-api-3jvu.onrender.com/gemini?prompt=${encodeURIComponent(c)}&uid=${d}`);
...
async function i(c) {
  try {
    const j = await axios.get(`https://deku-rest-api-3jvu.onrender.com/generate?prompt=${encodeURIComponent(c)}&style=3`);
    return j.data.url;
  } catch (k) {
    throw k;
  }
...
async function describeImage(prompt, photoUrl) {
  try {
    const url = `https://deku-rest-api-3jvu.onrender.com/describe?prompt=${encodeURIComponent(prompt)}&url=${encodeURIComponent(photoUrl)}`;
...
const a = {
  ...
  aliases: ["bard"],
  version: "4.0",
  author: "vex_kshitiz", //fixed by Alex
  countDown: 5,
  role: 0,
